...
public class OverlayMarker extends ViewGroup implements OverlayView, ClusterItem {
  ...
  private OverlayMarkerIcon iconView;
  ...
  public void setIconView(View iconView) {
    this.iconView = (OverlayMarkerIcon) iconView;
    if (marker != null) {
  ...
  private void addOverlay(BaiduMap baiduMap) {
    if (iconView != null) {
      iconView.updateDraw();
      iconBitmapDescriptor = BitmapUtil.createBitmapDescriptor(iconView, iconView.width, iconView.height);
    }
    ...
    float y = 26 * getResources().getDisplayMetrics().scaledDensity;
    option.yOffset((int) y);
